...
/// # edfplus::doctest_utils::create_simple_test_file("recording.bdf")?;
/// #
/// // Open an EDF+ file
/// let mut reader = EdfReader::open("recording.bdf")?;
///
...
/// # std::fs::remove_file("recording.bdf").ok();
/// # Ok::<(), edfplus::EdfError>(())
/// ```
...
///
/// # // Generate test file (hidden from docs)
/// # edfplus::doctest_utils::create_multi_channel_test_file("multi_signal.bdf")?;
/// #
/// let mut reader = EdfReader::open("multi_signal.bdf")?;
...
///
/// # // Cleanup (hidden from docs)
/// # std::fs::remove_file("multi_signal.bdf").ok();
/// # Ok::<(), edfplus::EdfError>(())
/// ```
...
impl EdfReader {
    ...
    /// match EdfReader::open("recording.bdf") {
    ///     Ok(reader) => {
    ...
    /// match EdfReader::open("nonexistent.bdf") {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        // Initialize the sample position pointer.
        let sample_positions = vec![0i64; header.signals.len()];

        // Parse annotations to obtain an accurate count of annotations and
        // potential sub-second timestamps.
    ...
    /// # edfplus::doctest_utils::create_simple_test_file("recording.bdf")?;
    /// #
    /// let reader = EdfReader::open("recording.bdf")?;
    /// let header = reader.header();
    ///
    ...
    /// # let mut writer = EdfWriter::create("test_annotations.bdf").unwrap();
    ...
    ///
    /// let reader = EdfReader::open("test_annotations.bdf").unwrap();
    ...
    /// # // Cleanup
    /// # drop(reader);
    /// # fs::remove_file("test_annotations.bdf").ok();
    /// ```
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_simple_test_file("eeg_recording.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("eeg_recording.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("eeg_recording.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_multi_channel_test_file("multi_channel.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("multi_channel.bdf")?;
    /// let signal_count = reader.header().signals.len();
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("multi_channel.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_validation_test_file("test.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("test.bdf")?;
    /// let signal_count = reader.header().signals.len();
    ///
    /// for i in 0..signal_count {
    ///     let signal_label = reader.header().signals[i].label.clone();
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("test.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    pub fn read_digital_samples(&mut self, signal: usize, count: usize) -> Result<Vec<i32>> {
        ...
        // Find the actual signal index (skip comment signals).
        ...
        // Calculate the maximum number of readable samples.
        ...
        // ✅ Performance Optimization: Employing a direct calculation approach similar to EDFLIB.
        while samples_read < actual_count {
            ...
            // Calculate the number of contiguous readable samples (avoiding cross-record boundaries).
            ...
            // ✅ Use a pre-calculated buffer_offset for direct positioning.
            let file_offset = self.header_size as u64
                + record_index as u64 * self.record_size as u64
                + signal_info.buffer_offset as u64
                + sample_in_record as u64 * 3; // BDF: 3 bytes per sample.

            // Navigate to the correct location.
            self.file.seek(SeekFrom::Start(file_offset))?;

            // ✅ Batch Reading to Improve Performance
            let bytes_to_read = samples_to_read * 3; // BDF: 3 bytes per sample.
            let mut buffer = vec![0u8; bytes_to_read];
            self.file.read_exact(&mut buffer)?;

            // Convert bytes to a numeric value and apply range limits.
            for chunk in buffer.chunks_exact(3) {
                let digital_value = i32::from_le_bytes([chunk[0], chunk[1], chunk[2], 0x00]);
                let digital_value = (digital_value << 8) >> 8;

                // let digital_value = i16::from_le_bytes([chunk[0], chunk[1]]) as i32;

                // ✅ Apply numerical range limits (similar to edflib's clamping)
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_simple_test_file("positioning.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("positioning.bdf")?;
    ///
    /// // Read from beginning
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("positioning.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_simple_test_file("bounds_test.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("bounds_test.bdf")?;
    /// let signal_length = reader.header().signals[0].samples_in_file;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("bounds_test.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_simple_test_file("position_test.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("position_test.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("position_test.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_multi_channel_test_file("multi_pos.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("multi_pos.bdf")?;
    /// let signal_count = reader.header().signals.len();
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("multi_pos.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_simple_test_file("rewind_test.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("rewind_test.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("rewind_test.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    ///
    /// # // Generate test file (hidden from docs)
    /// # edfplus::doctest_utils::create_simple_test_file("workflow.bdf")?;
    /// #
    /// let mut reader = EdfReader::open("workflow.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("workflow.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ...
    fn parse_header(reader: &mut BufReader<File>) -> Result<(EdfHeader, Vec<SignalInfo>, usize)> {
        ...
        // Verify BDF Identifier
        let version = String::from_utf8_lossy(&main_header[0..8]);
		// Char 255
        if main_header[0] != 0xFF {
            return Err(EdfError::UnsupportedFileType(format!("Not a BDF file: {}", version)));
        }

        // Parsing signal count
        ...
        // Check the BDF identifier.
        let reserved = String::from_utf8_lossy(&main_header[192..236]);
        if !reserved.starts_with("24BIT") {
            return Err(EdfError::UnsupportedFileType("Only BDF files are supported".to_string()));
        }
        ...
        // Parsing Data Record Information
        ...
        // Read Signal Header Information
        ...
        // Create a temporary header for comment parsing.
        ...
        // Parse annotations to obtain an accurate count of annotations and potential sub-second timestamps.
        let (annotations_count, starttime_subsecond) = Self::count_annotations_and_parse_subsecond(
            reader,
            &signal_info,
            datarecords,
    ...
    fn parse_signals(
        ...
        // Parse each field of every signal.
        for i in 0..total_signal_count {
            ...
            // Check if it is an annotation signal — it must exactly match "BDF Annotations" (note the trailing space).
            let is_annotation = label_bytes == b"BDF Annotations ";

            // Sensor Type (80 bytes, starting at offset 16 * signal_count)
            ...
            // Number of samples in each data record (8 bytes)
            ...
            // Creating SignalInfo — All signals must have the correct buffer_offset set.
            let info = SignalInfo {
                buffer_offset,  // Current cumulative byte offset
                samples_per_record,
                is_annotation,
            };

            // Only non-comment signals are added to the user-visible list of signals.
            if !is_annotation {
                // Validate Parameters
            ...
            // Add signal information to the list (including commented-out signals).
            signal_info.push(info);

            // ✅ Critical Fix: Update `buffer_offset` for all signals (including annotation signals).
            // Each sample occupies 3 bytes (fixed in the BDF format).
            buffer_offset += samples_per_record as usize * 3;
        }
    ...
    fn parse_annotations(&mut self) -> Result<Vec<Annotation>> {
        ...
        for record_idx in 0..datarecords {
            ...
            for (ann_idx, &ann_signal_idx) in annotation_signals.iter().enumerate() {
                ...
                // 提取注释信号数据
                let bytes_to_read = (ann_info.samples_per_record * 3 /* * 2 */) as usize;
    ...
    /// Calculate the number of annotations and parse sub-second time (if present).
    fn count_annotations_and_parse_subsecond(
        ...
        // Scan only the first few data records to calculate annotations (optimize performance).
        let records_to_scan = datarecords.min(100); // Scan up to 100 records.
        ...
        for record_idx in 0..records_to_scan {
            ...
            // Process each annotation signal.
            for (ann_idx, &ann_signal_idx) in annotation_signals.iter().enumerate() {
                let ann_info = &signal_info[ann_signal_idx];

                // Use the pre-calculated buffer_offset instead of recomputing it.
                let signal_offset = ann_info.buffer_offset;

                // Extract annotated signal data.  Samples per record by record size
                let bytes_to_read = (ann_info.samples_per_record * 3) as usize;
                if signal_offset + bytes_to_read <= record_data.len() {
                    let tal_data = &record_data[signal_offset..signal_offset + bytes_to_read];

                    // Parsing TAL Data to Calculate Annotations — Passing the Correct Annotation Signal Index
                    let (record_annotations, subsecond) = Self::quick_parse_tal_for_count(
                        tal_data,
                        record_idx == 0,
                        ann_idx == 0  // Only the first annotation signal is true.
                    )?;
                    annotation_count += record_annotations;

                    // The first record may contain sub-second information.
