...
/// # let mut writer = EdfWriter::create("test_header_example.bdf").unwrap();
...
///
/// let mut reader = EdfReader::open("test_header_example.bdf").unwrap();
...
/// # // Cleanup
/// # drop(reader);
/// # fs::remove_file("test_header_example.bdf").ok();
/// ```
pub struct EdfHeader {
    ...
    ///
    /// # // Create a test file first
    /// # let mut writer = EdfWriter::create("test_duration.bdf").unwrap();
    /// # writer.set_patient_info("P001", "M", "01-JAN-1990", "Test Patient").unwrap();
    /// # let signal = SignalParam {
    /// #     label: "EEG".to_string(),
    /// #     samples_in_file: 0,
    ...
    /// # writer.finalize().unwrap();
    ///
    /// let mut reader = EdfReader::open("test_duration.bdf").unwrap();
    /// let header = reader.header();
    ///
    ...
    /// # // Cleanup
    /// # drop(reader);
    /// # fs::remove_file("test_duration.bdf").ok();
    /// ```
