...
/// # fn main() -> Result<()> {
/// let mut writer = EdfWriter::create("test.bdf")?;
...
///
/// // Create new EDF+ file
/// let mut writer = EdfWriter::create("output.bdf")?;
///
...
/// # std::fs::remove_file("output.bdf").ok();
/// # Ok::<(), edfplus::EdfError>(())
/// ```
///
...
/// let mut writer = EdfWriter::create("multi_channel.bdf")?;
...
impl EdfWriter {
    ...
    /// let writer = EdfWriter::create("new_recording.bdf")?;
    /// println!("EDF+ writer created successfully");
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("new_recording.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    /// use edfplus::EdfWriter;
    ///
    /// match EdfWriter::create("/invalid/path/file.bdf") {
    ...
    /// use edfplus::{EdfWriter, SignalParam};
    ///
    /// let mut writer = EdfWriter::create("signals.bdf")?;
    ///
    /// // Add an EEG signal
    /// let eeg_signal = SignalParam {
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("signals.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    /// use edfplus::{EdfWriter, SignalParam};
    ///
    /// let mut writer = EdfWriter::create("mixed_rates.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("mixed_rates.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ...
    /// use edfplus::EdfWriter;
    ///
    /// let mut writer = EdfWriter::create("patient_data.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("patient_data.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    /// use edfplus::EdfWriter;
    ///
    /// let mut writer = EdfWriter::create("anonymous.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("anonymous.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    /// use edfplus::EdfWriter;
    ///
    /// let mut writer = EdfWriter::create("study_subject.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("study_subject.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ...
    /// use edfplus::EdfWriter;
    ///
    /// let mut writer = EdfWriter::create("custom_duration.bdf")?;
    ///
    /// // Set 0.5 second data records for higher temporal resolution
    /// writer.set_datarecord_duration(0.5)?;
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("custom_duration.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    /// use edfplus::EdfWriter;
    ///
    /// let mut writer = EdfWriter::create("high_freq.bdf")?;
    ///
    /// // Use 0.1 second records for fast neural signals
    /// writer.set_datarecord_duration(0.1)?;
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("high_freq.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ...
    fn write_header(&mut self, total_datarecords: i64) -> Result<()> {
        ...
        // 24 bits
        let annotation_samples_per_record = annotation_bytes_per_record / 3; // 每样本2字节

        for _ in 0..self.nr_annot_chns {
            annotation_signals.push(SignalParam {
                label: "BDF Annotations ".to_string(), // 标准要求的标签
        ...
        // EDF+标识 (44字节)
        // potentially needs: b"BDF+C";
        main_header[192..197].copy_from_slice(b"24BIT");

        for b in &mut main_header[197..236] {
            *b = b' ';
        }
    ...
    /// use edfplus::{EdfWriter, SignalParam};
    ///
    /// let mut writer = EdfWriter::create("multi_signal.bdf")?;
    ///
    ...
    /// use edfplus::{EdfWriter, SignalParam};
    ///
    /// let mut writer = EdfWriter::create("continuous.bdf")?;
    ///
    /// // Add a single signal
    /// writer.add_signal(SignalParam {
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("continuous.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    pub fn write_samples(&mut self, samples: &[Vec<f64>]) -> Result<()> {
        ...
        // Verify the number of samples for each signal.
        ...
        // If you haven't written the header yet, write it first.
        if !self.header_written {
            self.write_header(1)?; // Used temporarily; it will be updated during finalization.
        }

        // Write data in the order specified by EDFlib: signal data + annotation signals.

        // Write sample data for all signals.
        for signal_idx in 0..self.signals.len() {
            ...
            for &physical_value in signal_samples {
                ...
                let clamped_value = digital_value
                    ...
                    .min(signal.digital_max) as i32;

                // Written as 16-bit little-endian.
                // let bytes = (clamped_value as i16).to_le_bytes();
                // self.file.write_all(&bytes)?;

                // BDF - 3 bytes, little-endian signed 24-bit
                let bytes = clamped_value.to_le_bytes(); // 4 bytes
                self.file.write_all(&bytes[..3])?;       // write only the low 3
            }
        }

        // TAL Data for Writing Annotation Signals – Supports Multiple Annotation Channels
    ...
    /// use edfplus::{EdfWriter, SignalParam};
    ///
    /// let mut writer = EdfWriter::create("final_test.bdf")?;
    ///
    ...
    ///
    /// # // Cleanup (hidden from docs)
    /// # std::fs::remove_file("final_test.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    ///     let mut writer = EdfWriter::create("error_test.bdf")?;
    ...
    ///
    ///     # // Cleanup (hidden from docs)
    ///     # std::fs::remove_file("error_test.bdf").ok();
    ///     Ok(())
    /// }
    ///
    ...
    pub fn finalize(mut self) -> Result<()> {
        // If data has been written but the record count in the header is incorrect, the header must be updated.
        if self.header_written && self.samples_written > 1 {
            use std::io::{Seek, SeekFrom};

            // Flush the buffer to ensure that all data has been written.
            self.file.flush()?;

            // Obtain an internal file reference and seek to the position of the data record count (bytes 236–244).
    ...
    /// # fn main() -> Result<()> {
    /// let mut writer = EdfWriter::create("annotations.bdf")?;
    ...
    /// // Result: "This is a very long annotation descripti"
    /// # std::fs::remove_file("annotations.bdf").ok();
    /// # Ok(())
    /// # }
    /// ```
    ///
    ...
    /// # use std::fs;
    ///
    /// let mut writer = EdfWriter::create("annotations_test.bdf")?;
    /// writer.set_patient_info("P001", "M", "01-JAN-1990", "Test Patient")?;
    ///
    /// // Add a signal
    /// let signal = SignalParam {
    ...
    ///
    /// # // Cleanup
    /// # std::fs::remove_file("annotations_test.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ///
    ...
    /// # use std::fs;
    ///
    /// let mut writer = EdfWriter::create("sleep_study.bdf")?;
    ...
    ///
    /// # // Cleanup
    /// # std::fs::remove_file("sleep_study.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
    ...
    /// # use std::fs;
    ///
    /// let mut writer = EdfWriter::create("count_test.bdf")?;
    ...
    ///
    /// # // Cleanup
    /// # std::fs::remove_file("count_test.bdf").ok();
    /// # Ok::<(), edfplus::EdfError>(())
    /// ```
...
mod tests {
    ...
    fn test_edf_writer_default_annotation_settings() {
        let writer = EdfWriter::create("test_default.bdf").unwrap();
        assert_eq!(writer.nr_annot_chns, 1);
        cleanup_test_file("test_default.bdf");
    }

    #[test]
    fn test_set_number_of_annotation_signals() {
        let mut writer = EdfWriter::create("test_annot_num.bdf").unwrap();
        ...
        cleanup_test_file("test_annot_num.bdf");
    }

    #[test]
    fn test_modification_after_header_written() {
        let mut writer = EdfWriter::create("test_locked.bdf").unwrap();
        ...
        cleanup_test_file("test_locked.bdf");
    }

    #[test]
    fn test_multi_channel_annotation_header_creation() {
        let mut writer = EdfWriter::create("test_multi_header.bdf").unwrap();
        writer.set_patient_info("P001", "M", "01-JAN-1990", "Test").unwrap();
        ...
        assert!(Path::new("test_multi_header.bdf").exists());

        cleanup_test_file("test_multi_header.bdf");
    }

    #[test]
    fn test_annotation_tal_generation() {
        let mut writer = EdfWriter::create("test_tal.bdf").unwrap();
        writer.set_patient_info("P001", "M", "01-JAN-1990", "Test").unwrap();
        ...
        cleanup_test_file("test_tal.bdf");
    }

    #[test]
    fn test_complete_multi_channel_workflow() {
        let filename = "test_complete_workflow.bdf";
