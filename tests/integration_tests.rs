...
fn test_basic_write_read_cycle() {
    let filename = "test_basic_cycle.bdf";
...
fn test_multi_channel_recording() {
    let filename = "test_multi_channel.bdf";
...
fn test_different_sampling_rates() {
    let filename = "test_different_rates.bdf";
...
fn test_digital_vs_physical_values() {
    let filename = "test_digital_physical.bdf";
    ...
    // Reading Phase
    {
        ...
        // Reset position and read numeric value.
        reader.rewind(0).unwrap();
        ...
        // If the lengths do not match, it may be an implementation issue; start by checking the basic functionality.
        if digital_samples.len() == physical_samples.len() {


        // Verifying the Conversion of Known Values
